...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void TransformSelected()
		{
			...
			{
				if(nearAxis != Axis.None && !IsMouseOverInputField() && Input.GetMouseButtonDown(0))
				{
    ...
    foreach (Renderer render in renderers)
    {
        ...
        {
            // Create a new material array with two slots
            Material[] newMaterials = new Material[2];
            // Copy the original material to the first slot
            newMaterials[0] = materials[0];
            // Put the outlineMaterial in the second slot
            newMaterials[1] = outlineMaterial;
            render.materials = newMaterials;
        }
        else if (materials.Length > 1)
        {
            // Replace the second material in the array with outlineMaterial
...
void RemoveHighlightedRenderers(List<Renderer> renderers)
{
    foreach (Renderer render in renderers)
    {
        ...
        if (materials.Length > 1)
        {
            // Remove the second material from the array
